...
class Worker
  ...
  def handle_job_assign_uniq(data, sock, hostport)
    handle, func, uid, data = data.split("\0", 4)
    if not func
      Util.logger.error "GearmanRuby: Ignoring job_assign_uniq with no function from #{hostport}"
      return false
    end

    Util.logger.error "GearmanRuby: Got job_assign_uniq with handle #{handle} and #{data.size} byte(s) and uid #{uid} " +
      "from #{hostport}"
  ...
  ##
  # Do a single job and return. Is aware of the unique ID assigned to a task by a client
  def work_uniq
    req = Util.pack_request(:grab_job_uniq)
    loop do
      @status = :preparing
      bad_servers = []
      ...
      servers.each do |hostport|
        Util.logger.debug "GearmanRuby: Sending grab_job_uniq to #{hostport}"
